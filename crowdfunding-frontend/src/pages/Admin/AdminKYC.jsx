      ...
      <div className="bg-white rounded-xl shadow-xl max-w-lg w-full p-6 animate-in fade-in zoom-in duration-200">
        ...
        <div className="space-y-4">
          ...
          <div className="bg-blue-50 text-blue-800 p-3 rounded-lg border border-blue-200 flex items-start gap-3 mt-4">
             ...
             <p className="text-sm">Assurez-vous que le document correspond au nom de l'utilisateur. Toute validation est enregistrée dans les registres d'audit.</p>
          </div>
        </div>
...
export default function AdminKYC() {
  ...
  const filteredDocs = documents.filter((doc) => {
    const searchString = `${doc.typeDocument} ${doc.statut}`.toLowerCase();
